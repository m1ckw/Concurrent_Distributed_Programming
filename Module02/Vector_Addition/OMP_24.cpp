...
int main(){
    ...
    // Parallelise population of vectors and sum the values.
    cout << "Running Threads";
    #pragma omp parallel shared(v1, v2, v3, size, total) firstprivate(localTotal) default(none)
    {
        ...
        {
            v3[i] = v1[i] + v2[i];
            // Update using reduction
            localTotal += v3[i];
        }

        // add localTotal to total one thread at a time
        #pragma omp critical
        total += localTotal;
    }
    ...
    // Print Summary
    cout << ",\nOMP: Calcs private total then adds to global total: 2.3 ("
    << THREADS << " Threads, " <<  size << " Elements). \nTotal Value: "
